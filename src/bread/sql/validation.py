...
class DataValidator:
    ...
    def validate(self, table_name: str, data: Dict[str, Any]) -> bool:
        ...
        Performs checks for table existence, valid column names, non-nullable constraints, and
        correct data types.
        ...
        for col in tbl.columns:
            ...
            # Ignore columns with Generated Identity set to ALWAYS as they do not take any values
            if col.generated_identity == Opts.ALWAYS:
                continue

            # Check 1: Presence. A non-nullable column with no set default value must be in the provided data.
            if (col.not_null
                    and col.default is None
                    and col.generated_identity is None
                    and column_name not in provided_cols):
            ...
            if value is not None and not isinstance(value, col.sql_type.py()):
                print(
                    ...
                    f"expected {col.sql_type.py().__name__}, got {type(value).__name__}")
                return False
        return True
